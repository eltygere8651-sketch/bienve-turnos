...
const Summary: React.FC<SummaryProps> = ({ totalHours, overtimeHours, onDownload, isDownloading }) => {
    return (
        <footer className="sticky bottom-0 bg-gray-800/80 backdrop-blur-sm border-t border-gray-700 p-4 shadow-top">
            <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0 sm:space-x-4">
                ...
                <button
                    ...
                    className="flex items-center space-x-2 px-6 py-3 bg-red-600 hover:bg-red-500 text-white font-semibold rounded-lg shadow-md transition-transform transform hover:scale-105 duration-300 disabled:bg-gray-500 disabled:cursor-not-allowed disabled:scale-100"
                >
                    <DownloadIcon className="w-5 h-5" />
                    <span>{isDownloading ? 'Generando...' : 'Descargar Horario'}</span>
                </button>
            </div>
        </footer>
    );
